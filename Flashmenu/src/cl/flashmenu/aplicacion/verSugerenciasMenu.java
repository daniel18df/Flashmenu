...
public class verSugerenciasMenu extends ListActivity{
	...
	DecimalFormat fmt;
	private static String url_Lista_platos = servidor.ip() + servidor.ruta2()+"verSugerenciasMenu.php";
	...
	class LoadAllplatos extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(verSugerenciasMenu.this);
			pDialog.setMessage("Cargando menus. Please wait...");
...
//			params.add(new BasicNameValuePair("buscar", UserData.idCliente));
			...
			try {
				...
				if (success == 1) {
					...
					sugerencias = json.getJSONArray(UserData.TAG_SugerenciasMenu);

					for (int i = 0; i < sugerencias.length(); i++) {
						JSONObject c = sugerencias.getJSONObject(i);

						UserData.idMenu = c.getString(UserData.TAG_ID_MENU);
						UserData.Menu_precio = c.getString(UserData.TAG_MENU_PRECIO);
						UserData.Menu_nombre = c.getString(UserData.TAG_MENU_NOMBRE);
						UserData.Menu_precio = c.getString(UserData.TAG_MENU_PRECIO);
						UserData.SProducto_nombre = c.getString(UserData.TAG_NOMBRE_PRODUCTO);
						UserData.idRestaurant	= c.getString(UserData.TAG_ID_PRODUCTO_REST);
						...
						map.put(UserData.TAG_MENU_NOMBRE, UserData.Menu_nombre);
						map.put(UserData.TAG_MENU_PRECIO, UserData.Menu_precio);
			...
			runOnUiThread(new Runnable() {
				public void run() {
					...
					ListAdapter adapter = new SimpleAdapter(
							verSugerenciasMenu.this, SuegerenciasList, R.layout.list_itemplatos,
							new String[] { UserData.TAG_MENU_NOMBRE, UserData.TAG_NOMBRE_PRODUCTO, UserData.TAG_MENU_PRECIO, UserData.TAG_ID_REST}, new int[] {R.id.nombrePlato, R.id.descripcionPlato, R.id.precioPlato, R.id.cantidadPlatosSeleccionados});
